...
#sys.path.append('/content/drive/MyDrive/Colab_Notebooks/deepreach-2D_case')
...
model = modules.SingleBVPNet(in_features=4, out_features=1, type=opt.model, mode=opt.mode, final_layer_factor=1.,
                             hidden_features=opt.num_nl, num_hidden_layers=opt.num_hl)
model.cuda()

# Define the loss
loss_fn = loss_functions_hard_constraints.initialize_hji_air3D_exact_hard_cons(dataset, opt.minWith)
